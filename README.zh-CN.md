...
4. 先用 `docs/skills/study-card-maintainer.md` 作为基础 skill。
5. 再叠加 `docs/skills/` 里最接近的领域 skill。
6. 只更新 `data/` 下的可发布内容，以及 `audio/` 下的可选音频。
7. 运行 `node scripts/validate-content.js`。
8. commit 或 publish 前必须请人类确认。
...
- 与模型无关的 agent workflow 文档和可直接复制给 agent 的 starter skills。
...
App 是纯静态 HTML/CSS/JS，校验脚本只使用 Node 内置模块。
...
- `docs/skills/`：基础、Markdown 和不同学习场景的 starter skills。
